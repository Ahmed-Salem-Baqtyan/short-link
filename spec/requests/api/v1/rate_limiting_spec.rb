...
RSpec.describe 'Rate Limiting', type: :request do
  ...
  describe 'Cache backend verification' do
    it 'uses Rails.cache for rate limiting' do
      expect(Rails.cache).to receive(:read).at_least(:once).and_call_original
      expect(Rails.cache).to receive(:write).at_least(:once).and_call_original
      
      params = { short_url: { url: 'https://example.com/cache-test' } }
      post('/api/v1/short_urls/encode', params: params.to_json, headers: valid_headers)
    end

    it 'uses Solid Cache in test environment' do
      expect(Rails.cache.class.name).to eq('ActiveSupport::Cache::SolidCacheStore')
    end

    it 'stores rate limit counters in database' do
      # Make a request to trigger rate limiting
      params = { short_url: { url: 'https://example.com/cache-test' } }
      post('/api/v1/short_urls/encode', params: params.to_json, headers: valid_headers)
      
      # Verify cache entry was created in solid_cache_entries table
      expect(SolidCache::Entry.count).to be > 0
    end
  end
end
